...
try:
    ...
    def zakup(bal, pc):
        ...
        WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "[type='number']"))).send_keys(str(round((bal-podushka[0])*pc, 2)))
        time.sleep(1)
    ...
    def check(n, bal):
        if perc[n]!=0:
            ...
            if save==True:
                ...
                file.write("Бот не сделал ставку" + "\n")
                file.close()
            else:
                file=open("history.txt", 'a')
